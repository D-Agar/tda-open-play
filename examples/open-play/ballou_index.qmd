...
```{r}
#| label: data-read
intake <- read_csv("data/clean/survey_intake.csv.gz")
...
daily_raw <- read_csv("data/raw/survey_daily_raw.csv.gz")
...
prop_dropped_timeuse <- number(
  (1 - (nrow(time_use) / nrow(read_csv("data/raw/timeuse_raw.csv.gz")))) * 100,
  accuracy = 0.1
)
...
steam_qc <- read_csv(
  "data/qc/steam_quality_metrics.csv",
...
median_minutes_reduced <- number(
  steam_qc$median_minutes_reduced,
  accuracy = 0.1
)
```
  ...
  mutate(
    ...
    stage2 = case_when(
      is.na(linked_platforms) &
        is.na(eligible_platforms) ~ "Ineligible platforms",
      is.na(linked_platforms) &
        !is.na(eligible_platforms) ~ "Elected not to link",
      TRUE ~ "Linked ≥1 account"
    ),
    stage3 = ifelse(
...
sankey_data <- intake |>
  mutate(
    ...
    `Account\nValidation 1` = ifelse(
      pid %in% c(xbox$pid, nintendo$pid, steam$pid) | qualified,
      "Any trace data",
      "No trace data"
    ),
    `Account\nValidation 2` = ifelse(
...
#| tbl-cap: "Data availability and missingness. Left columns show qualified participants (with recent telemetry + self-reported platform), total with data, and drop-off. Right columns show active participants (1+ surveys), observations, and missingness among active participants."
## ---- constants -------------------------------------------------------------
...
## ---- qualified counts (measure-specific) -----------------------------------
n_qualified <- c(
  "Daily surveys" = sum(intake$qualified & intake$country == "US"),
  "Biweekly surveys" = sum(intake$qualified),
  "Time use diaries" = sum(intake$qualified & intake$country == "US"),
  "Attention tasks" = sum(intake$qualified),
  "Xbox" = sum(
    intake$qualified & str_detect(intake$used_platforms, "Xbox"),
    na.rm = TRUE
  ),
  "Nintendo" = sum(
    intake$qualified & str_detect(intake$used_platforms, "Nintendo"),
    na.rm = TRUE
  ),
  "Steam" = sum(
    intake$qualified & str_detect(intake$used_platforms, "Steam"),
    na.rm = TRUE
  ),
  "iOS" = sum(
    intake$qualified & str_detect(intake$used_platforms, "iOS"),
    na.rm = TRUE
  ),
  "Android" = sum(
    intake$qualified & str_detect(intake$used_platforms, "Android"),
    na.rm = TRUE
  )
)
...
qualified_pids <- intake$pid[intake$qualified]
intake_pids <- intake$pid
...
rows <- list(
  # ---- Surveys ----
  {
    ...
    tibble(
      ...
      n_qualified = n_qualified["Daily surveys"],
      n_total_all = daily |>
        filter(pid %in% intake_pids) |>
        distinct(pid) |>
        nrow(),
      n_total_qual = daily |>
        filter(pid %in% qualified_pids) |>
  ...
  },
  {
    ...
    tibble(
      ...
      n_qualified = n_qualified["Biweekly surveys"],
      n_total_all = biweekly |>
        filter(pid %in% intake_pids) |>
        distinct(pid) |>
        nrow(),
      n_total_qual = biweekly |>
        filter(pid %in% qualified_pids) |>
        distinct(pid) |>
        nrow(),
  ...
  },
  {
    ...
    tibble(
      ...
      n_qualified = n_qualified["Time use diaries"],
      n_total_all = time_use |>
        filter(pid %in% intake_pids) |>
        distinct(pid) |>
        nrow(),
      n_total_qual = length(pids_timeuse),
  ...
  },
  {
    ...
    tibble(
      data_type = "Telemetry",
      measure = "Nintendo",
      n_qualified = n_qualified["Nintendo"],
      n_total_all = nintendo |>
        filter(pid %in% intake_pids) |>
        distinct(pid) |>
        nrow(),
      n_total_qual = nintendo |>
        filter(pid %in% qualified_pids) |>
        distinct(pid) |>
        nrow(),
      n_participants = length(p_elig),
  ...
  },
  {
    ...
    tibble(
      data_type = "Telemetry",
      measure = "Android",
      n_qualified = n_qualified["Android"],
      n_total_all = android |>
        filter(pid %in% intake_pids) |>
        distinct(pid) |>
        nrow(),
      n_total_qual = android |>
        filter(pid %in% qualified_pids) |>
        distinct(pid) |>
        nrow(),
      n_participants = length(p_elig),
  ...
  # ---- Task ----
  {
    ...
    tibble(
      ...
      n_qualified = n_qualified["Attention tasks"],
      n_total_all = simon |>
        filter(pid %in% intake_pids) |>
        distinct(pid) |>
        nrow(),
  ...
  list_rbind() |>
  arrange(
    factor(data_type, levels = c("Survey", "Task", "Digital Trace Data")),
    factor(
      measure,
      levels = c(
        "Daily surveys",
...
table_data <- rows |>
  mutate(
    ...
    pct_dropoff_num = 100 * (1 - n_total_qual / n_qualified),
    `N qual.` = n_fmt(n_qualified),
    `N data` = n_fmt(n_total_all),
    `% Drop` = sprintf("%.1f%%", pct_dropoff_num),
    `N active` = n_fmt(n_participants),
    `N obs.` = n_fmt(total_observed),
    `Max obs.` = n_fmt(maximum_possible),
    `% Miss.` = sprintf("%.1f%%", pct_missing_num)
  ) |>
  group_by(data_type) |>
  ...
  mutate(Type = if_else(show_type, Type, "")) |>
  select(-show_type)
...
n_qual_text <- "N qual. = qualified participants. For digital trace data, qualified participants who self-reported using that platform."
n_data_text <- "N data = all participants with data (may include non-qualified)."
drop_text <- "% Drop = percentage of qualified participants without data."
max_obs_text <- "Max obs.: Daily survey = N × 30 days; Biweekly survey = N × 6 waves; Time use = N × 30 days; Attention tasks = N × 3 administrations; Xbox/Nintendo = N participants with ≥1 session; Steam = N × 84 days × 24 hours (visibility hours); iOS/Android = N × 84 days."

tt(
  table_data,
  notes = list(
    "a" = list(i = 0, j = 3, text = n_qual_text),
    "b" = list(i = 0, j = 4, text = n_data_text),
    "c" = list(i = 0, j = 5, text = drop_text),
    "d" = list(i = 0, j = 8, text = max_obs_text)
  ),
  width = 1,
  escape = TRUE
) |>
  ...
  style_tt(j = 5, line = "r", line_width = 0.08) |>
  style_tt(fontsize = 0.7)
```
:::
