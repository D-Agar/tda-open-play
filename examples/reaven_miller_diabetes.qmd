...
raw_scaled <- data.frame(scale(raw_data, center = FALSE))
...
```{r low_res_MapperAlgo}
library(MapperAlgo)
l2_dist <- as.matrix(dist(raw_scaled, method = "euclidean"))
density_est <- density_estimation(l2_dist)
densities <- density_est$values

low_res_mapper <- MapperAlgo(
  original_data = raw_data,
  filter_values = densities,
  percent_overlap = 50,
  methods = "hierarchical",
  method_params = list(num_bins_when_clustering = 4, method = "single"),
  cover_type = "stride",
  intervals = 3
)
...
density_lens <- density_est$values
